import Link from "next/link";

export default function HomePage() {
  return (
    <section className="relative overflow-hidden rounded-3xl border border-border bg-surface/40 px-6 py-16 md:px-12">
      <div className="absolute inset-0 bg-atmosphere opacity-90" />
      <div className="relative z-10 mx-auto flex max-w-5xl flex-col gap-10">
        <div className="animate-fade-up">
          <p className="text-xs uppercase tracking-[0.3em] text-text-secondary">
            Business interview practice
          </p>
          <h1 className="mt-4 text-4xl font-semibold leading-tight md:text-6xl">
            Train the exact skills you need for case interviews and business
            prompts.
          </h1>
          <p className="mt-4 max-w-2xl text-base text-text-secondary md:text-lg">
            Run timed drills, speak or type your response, then compare against a
            rubric and strong sample answers. Build confidence across
            estimations, behaviorals, and reasoning puzzles.
          </p>
          <div className="mt-6 flex flex-wrap items-center gap-4">
            <Link
              href="/problems"
              className="rounded-full border border-accent bg-accent px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-accent/30 transition hover:bg-accent/90"
            >
              Start practicing
            </Link>
            <Link
              href="/dashboard"
              className="rounded-full border border-border bg-surface px-6 py-3 text-sm font-semibold text-text-primary transition hover:border-accent/50"
            >
              View dashboard
            </Link>
          </div>
        </div>
        <div className="grid gap-6 md:grid-cols-3">
          {[
            {
              title: "Estimations",
              copy: "Market sizing, cost, and volume drills with calibrated timing.",
            },
            {
              title: "Behaviorals",
              copy: "Structured prompts that sharpen leadership and judgment stories.",
            },
            {
              title: "Reasoning",
              copy: "Logic puzzles to test structure, clarity, and communication.",
            },
          ].map((card) => (
            <div
              key={card.title}
              className="rounded-2xl border border-border bg-background/60 p-5"
            >
              <h3 className="text-lg font-semibold text-text-primary">
                {card.title}
              </h3>
              <p className="mt-2 text-sm text-text-secondary">{card.copy}</p>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
}
